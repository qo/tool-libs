    ...
    while (!decodingComplete) {
        ...
        if (isVideo) {
            ...
            for (uint j = 0; j < packetsCount; ++j) { // for (int j=0; j < 1; ++j) { // проверка проигрывания одного кадра
                ...
                r = nestegg_packet_data(packet, j, &frameData, &frameLength);
                assert(r == 0);
                ...
                if (_withAlpha){
                    r = nestegg_packet_additional_data(packet, 1, &alphaData, &alphaLength);
                }
                
                // Выполнение декодирования кадра
                // CODE HERE!!!
                r = vpx_codec_decode(&_normalCodec, frameData, frameLength, NULL, 0);
                
                // Если есть альфа, то декодим альфу
                if (_withAlpha) {
                    // Выполнение декодирования альфы кадра
                    // CODE HERE!!!
                    r = vpx_codec_decode(&_alphaCodec, alphaData, alphaLength, NULL, 0);
                }
            }
            ...
            if (_withAlpha) {
                ...
                auto frame = vpx_codec_get_frame(&_normalCodec, &iterator);
                
                vpx_codec_iter_t alphaIterator = NULL;
                auto alphaFrame = vpx_codec_get_frame(&_alphaCodec, &alphaIterator);
                // printf("Dimensions %d %d\n", alphaFrame->planes[VPX_PLANE_V], alphaFrame->planes[VPX_PLANE_Y]);

                libyuv::I420AlphaToARGB(frame->planes[VPX_PLANE_Y], frame->stride[VPX_PLANE_Y], 
                        frame->planes[VPX_PLANE_U], frame->stride[VPX_PLANE_U], 
                        frame->planes[VPX_PLANE_V], frame->stride[VPX_PLANE_V], 
                        alphaFrame->planes[VPX_PLANE_Y], alphaFrame->stride[VPX_PLANE_Y], _decodedBuffer, 
                        _nestegVideoParams.width * 4, frame->d_w, frame->d_h, 0);
                ...
                libyuv::I420ToRAW(frame->planes[VPX_PLANE_Y], frame->stride[VPX_PLANE_Y], frame->planes[VPX_PLANE_U], frame->stride[VPX_PLANE_U], 
                        frame->planes[VPX_PLANE_V], frame->stride[VPX_PLANE_V], _decodedBuffer, 
                        _nestegVideoParams.width * 3, frame->d_w, frame->d_h);
                // printf("Sample at 400 %d\n", _decodedBuffer[400 * 5 * 3]);
                // printf("Dimensions %d %d\n", frame->d_w, frame->d_h);
            }
            // xgen::video::yuv420ToRGB(frame->w, frame->h, frame->planes[0], frame->planes[1], 
            //             frame->planes[2], frame->stride[0], frame->stride[1], 
            //             frame->stride[2], _decodedBuffer);
...
void WebMVideoDecoder::copyDataToTexture(uint textureid){
    ...
    if (_decodedBuffer) {
        // CODE HERE!!!
        // printf("Video Dimms %d %d\n", _nestegVideoParams.width, _nestegVideoParams.height);
        glBindTexture(GL_TEXTURE_2D, textureid);
        if (_withAlpha){
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _nestegVideoParams.width, _nestegVideoParams.height, 0, GL_RGBA,
                GL_UNSIGNED_BYTE, _decodedBuffer);
        }
        else {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, _nestegVideoParams.width, _nestegVideoParams.height, 0, GL_RGB,
                GL_UNSIGNED_BYTE, _decodedBuffer);
        }
    }
...
void WebMVideoDecoder::shiftFrames(int shiftVector){
    int r = 0;
    nestegg_packet* packet = nullptr;
    ...
    // r = nestegg_packet_track(packet, &track);
    // // r = nestegg_tstamp_scale(_nesteg, &_timeStampScale);
    // r = decoderSeek(0, NESTEGG_SEEK_CUR, _nesteg);
    uint64_t duration;
    nestegg_duration(_nesteg, &duration);

    // for (uint i = 0; i < _tracksCount; ++i){
        r = nestegg_track_seek(_nesteg, 0, curTimeStamp + 1000000);
    // }
